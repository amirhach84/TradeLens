...
export default function App() {
  ...
  const DashboardPage = () => {
    const monthRows = [];
    for(let i=0;i<12;i+=4) monthRows.push(MONTHS.slice(i,i+4).map((_,j)=>i+j));
    return (
      <>
        {/* Year strip - 4 per row on mobile, 12 on desktop */}
        <div style={{background:C.card,borderRadius:16,padding:isMobile?"12px":"16px",marginBottom:14,border:`1px solid ${C.border}`}}>
          ...
          {/* Mobile: 3 rows of 4, Desktop: 1 row of 12 */}
          {isMobile ? (
                      ...
                      <div key={i} onClick={()=>setSelectedMonth(i)}
                        ...
                        {d&&d.trades>0?(
                          <>
                            ...
                            <div style={{fontSize:9,color:C.text2}}>{d.trades}t</div>
                          </>
              ...
              {MONTHS.map((m,i) => {
                ...
                return (
                  <div key={i} onClick={()=>setSelectedMonth(i)}
                    ...
                    {d&&d.trades>0?(
                      <>
                        <div style={{fontSize:11,fontWeight:800,color:pnl>=0?C.green:C.red}}>{fmt(pnl)}</div>
                        <div style={{fontSize:9,color:C.text2}}>{d.trades}t</div>
                      </>
          ...
          <div style={{background:C.card,borderRadius:16,padding:isMobile?"12px":"16px",border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={()=>{const m=selectedMonth===0?11:selectedMonth-1;setSelectedMonth(m);if(selectedMonth===0)setCalYear(y=>y-1);}}
                  style={{background:C.card2,border:"none",color:C.text2,borderRadius:6,padding:"3px 8px",cursor:"pointer"}}>‹</button>
                <button onClick={()=>{const m=selectedMonth===11?0:selectedMonth+1;setSelectedMonth(m);if(selectedMonth===11)setCalYear(y=>y+1);}}
                  style={{background:C.card2,border:"none",color:C.text2,borderRadius:6,padding:"3px 8px",cursor:"pointer"}}>›</button>
              </div>
            </div>
              ...
              {Array.from({length:daysInMonth}).map((_,i)=>{
                ...
                return (
                  <div key={day} style={{background:bg,borderRadius:5,padding:"3px 2px",textAlign:"center",minHeight:isMobile?38:48,border:`1px solid ${hasTrades?pnl>=0?"rgba(34,197,94,0.2)":"rgba(239,68,68,0.2)":C.border}`}}>
                    ...
                    {hasTrades&&(
                      <>
                        <div style={{fontSize:isMobile?8:9,fontWeight:800,color:pnl>=0?C.green:C.red}}>{fmt(pnl)}</div>
                        <div style={{fontSize:8,color:C.text3}}>{d.trades}t·{Math.round(d.wins/d.trades*100)}%</div>
                      </>
                    )}
                  </div>
                );
        ...
        <div style={{background:C.card,borderRadius:14,padding:"14px 16px",border:`1px solid ${C.border}`}}>
          ...
          <div style={{background:`linear-gradient(135deg,rgba(99,102,241,0.08),rgba(6,182,212,0.04))`,borderRadius:10,padding:12,border:"1px solid rgba(99,102,241,0.15)"}}>
            {aiInsights().length===0
              ?<div style={{fontSize:13,color:C.text2}}>Not enough data for insights.</div>
        ...
        <>
          ...
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(3,1fr)",gap:12,marginBottom:14}}>
            {[
              ...
              <div key={i} style={{background:C.card,borderRadius:14,padding:16,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:10,color:C.text3,marginBottom:6}}>{k.label}</div>
                <div style={{fontSize:isMobile?24:30,fontWeight:800,color:k.color}}>{k.value}</div>
              </div>
            ))}
          </div>
          {aiInsights().map((ins,i)=>(
            <div key={i} style={{background:C.card,borderRadius:14,padding:14,marginBottom:10,border:`1px solid ${ins.color}44`}}>
          ...
          <div style={{background:`linear-gradient(135deg,rgba(99,102,241,0.1),rgba(6,182,212,0.06))`,borderRadius:16,padding:16,border:"1px solid rgba(99,102,241,0.2)"}}>
            ...
            <div style={{fontSize:13,color:C.text2,lineHeight:1.7}}>
              ...
              <br/><br/>
              {tiltData.wrAfterL<40&&"⚠️ WR drops after losses. Stop after 2 losses/day."}
